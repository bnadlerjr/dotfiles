...
This template defines the structure for research documents generated by the `/research` skill.

## Required Metadata

Before using this template, gather metadata via `git metadata`:
...
`$(claude-docs-path research)/YYYY-MM-DD-description.md`
...
### Frontmatter
- All fields required
- Get `Area` from `git metadata` output
- Ask user for `Project` name via AskUserQuestion
