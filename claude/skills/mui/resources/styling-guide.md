...
The `sx` prop is the primary styling method in MUI (all versions):
...
**Note:** CSS variables are experimental in v5, stable in v6+.

```typescript
// In theme configuration
const theme = createTheme({
  cssVariables: true  // Experimental in v5, stable in v6+
});
...
// v7+ supports CSS layers for Tailwind integration
const theme = createTheme({
  cssVariables: {
    enableCssLayer: true  // v7+ only
  }
});
```
