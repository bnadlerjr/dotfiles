...
This reference focuses exclusively on safe database migration patterns: avoiding table locks, preventing blocked reads/writes, and ensuring zero-downtime schema changes. It does not cover schema design, query optimization, or application-level Ecto patterns — see [ecto-database](ecto-database.md) for those topics.
